...
from utils.gemini_helper import generate_content, format_course_outline
...
if submitted:
    if not all([title, subject, audience, duration]):
        ...
        with st.spinner("🎨 Crafting your course outline..."):
            outline = generate_content(full_prompt)
        
        if outline:
            st.success("✅ Course outline generated successfully!")
            
            # Display outline in styled tabs
            tab1, tab2, tab3 = st.tabs(["📄 Outline", "📊 Structure", "💾 Export"])
            ...
            with tab2:
                ...
                for module, percentage in progress_data.items():
                    st.markdown(f"**{module}**")
                    st.progress(percentage / 100)
                    st.caption(f"{percentage}% of course")
